# Make_Kashi_Sheets.ps1 - condensed photo sheets of S. Kashi & Sons stock on hand.
#
# Reads kashi_items.csv (next to this script), finds each item's photo in the
...
#     Make_Kashi_Sheets.ps1 -ImageFolder "N:\images"
#
# -Csv <file>     use a different item list, same column names
...
param([string]$ImageFolder, [string]$Csv, [int]$ThumbWidth = 150, [switch]$Auto)
...
if (-not $Csv) { $Csv = Join-Path $here 'kashi_items.csv' }
if (-not (Test-Path $Csv)) { Say "$Csv not found." 'Red'; if(-not $Auto){Read-Host 'Enter'}; exit 1 }
...
foreach ($st in $stores) {
    ...
    [void]$sb.Append("<html><head><meta charset='utf-8'><title>$st S Kashi</title>$CSS</head><body>")
    [void]$sb.Append("<h1>$st - S. Kashi &amp; Sons on hand</h1>")
    [void]$sb.Append("<div class='sub'>$($rows.Count) pieces &nbsp;&bull;&nbsp; `$$('{0:N0}' -f $cost) at cost &nbsp;&bull;&nbsp; `$$('{0:N0}' -f $ret) at retail &nbsp;&bull;&nbsp; oldest first. Red age means 3 years or more. MEMO means the vendor still owns it.</div><div class='grid'>")
    $n=0; $miss=0
    foreach ($r in $rows) {
        $n++
        $b64=$null
        ...
        $memo = if ($r.owned -eq 'MEMO') { " <span class='memo'>MEMO</span>" } else { '' }
    ...
    $out = Join-Path $here "Kashi_$st.html"
