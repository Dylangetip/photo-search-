...
W=7.0*inch; B=25000.0

# ---- data -------------------------------------------------------------
c=pd.read_pickle('lg/cats_spec.pkl').sort_values('ret6m',ascending=False).reset_index(drop=True)
DROP=['Watches - other brands']      # removed from the report entirely
MEMO=['Loose lab-grown diamonds']    # taken on memo: no cash tied up, so not in the split
NICE={'Anniversary bands - lab':'Lab anniversary bands',
      ...
      'Loose natural diamonds':'Loose natural diamonds'}
c=c[~c.category.isin(DROP)].reset_index(drop=True)
c['label']=c.category.map(lambda x: NICE.get(x,x))
NCAT=len(c)

avail=c[~c.category.isin(MEMO)]
rem=B; split=[]
for _,r in avail.iterrows():
    if rem<=0.5: break
    take=min(r['cap'],rem)
    if take<=0.5: continue
    split.append(dict(label=r['label'],d=take,rt=r['ret6m'],p=take*r['ret6m'],
                      u=take/r['avg_cost'],days=r['med_days']))
    rem-=take
TP=sum(s['p'] for s in split); TU=sum(s['u'] for s in split)
...
story.append(box(
 ...
 '<b>' + m(2609) + '</b>.<br/><br/>'
 '<b>3. But rings alone are not the best use of the money.</b> Spread the same $25,000 across the four best '
 'categories we actually pay for and six month profit is <b>' + m(TP) + ', a ' + '{:.0f}'.format(100*TP/B) +
 '% return</b>. Citizen ranks last of ' + str(NCAT) + '.<br/><br/>'
 '<b>4. Loose lab grown diamonds are left out of the split on purpose.</b> We take them on memo, so they '
 'tie up no cash and do not compete for the $25,000. Keep taking them.'))
...
rows=[['Category','Profit per $1\nin 6 months','Most we could\nput into it','Typical\ndays','Margin']]
cit_row=None
for i,r in c.iterrows():
    if r['category'] in MEMO: cap='on memo, no cash'
    elif r['cap']>=B:         cap='over $25,000'
    else:                     cap=m(r['cap'])
    rows.append([r['label'],'{:.0f}¢'.format(round(r['ret6m']*100)),cap,
...
story.append(sec(
 ...
 Paragraph('<b>"Most we could put into it"</b> is the size of order a category has actually been able to '
           'clear in a typical six months, plus 50% headroom. It matters as much as the return: a category '
           'that pays 50&#162; on the dollar but can only absorb $2,000 cannot do anything with $25,000.', NOTE),
 Spacer(1,5),
 grid(rows,[2.5*inch,1.15*inch,1.4*inch,.95*inch,1.0*inch],fs=9,hfs=8.5,
      extra=[('BACKGROUND',(0,cit_row),(-1,cit_row),colors.HexColor('#FBE9E9'))])))

# ---------------- PAGE 3 ----------------
story.append(PageBreak())
srows=[['Put this much into...','Dollars','Share','Pieces','Profit per $1','6 month profit','Days']]
for s in split:
    srows.append([s['label'],m(s['d']),'{:.0f}%'.format(100*s['d']/B),'~{:.0f}'.format(round(s['u'])),
                  '{:.0f}¢'.format(round(s['rt']*100)),m(s['p']),'{:.0f}'.format(s['days'])])
srows.append(['TOTAL',m(B),'100%','~{:.0f}'.format(round(TU)),
              '{:.0f}¢'.format(round(100*TP/B)),m(TP),''])
story.append(sec(
 Paragraph('THE BEST SPLIT OF $25,000', HEAD),
 box('Six month profit: <b>' + m(TP) + ', a ' + '{:.0f}'.format(100*TP/B) + '% return</b>. Fill the three '
     'small lab categories to what they can absorb, then put the rest into necklaces, the only category left '
     'that both pays well and is deep enough to take real money.'),
 Spacer(1,7),
 grid(srows,[1.95*inch,.95*inch,.65*inch,.65*inch,.95*inch,1.05*inch,.6*inch],fs=9,hfs=8.5,
      extra=[('BACKGROUND',(0,len(srows)-1),(-1,len(srows)-1),GREEN),
             ('FONT',(0,len(srows)-1),(-1,len(srows)-1),'Helvetica-Bold',9)]),
 Spacer(1,5),
 Paragraph('This is deliberately all merchandise we pay for. Loose lab grown diamonds would rank third on '
           'return, but we take them on memo and owe nothing until they sell, so they belong outside the '
           'budget rather than inside it. The same $25,000 in Citizen returns about ' +
           m(min(CT['cap'],B)*CT['ret6m']) + ', because the category can only absorb ' + m(CT['cap']) +
           ' of it. Even pretending it could take the whole amount it would return ' + m(B*CT['ret6m']) + '.', NOTE)))

story.append(sec(
 Paragraph('WHAT I WOULD DO', HEAD),
 box('<b>1. Stop buying Citizen.</b> Last of ' + str(NCAT) + ' on return, slowest to turn at 315 days, and '
     'half of what is left has been in the case over two years.<br/><br/>'
     '<b>2. Buy lab anniversary bands until they stop selling.</b> Best return in the building at 50&#162; and a '
     '92 day turn, but we have only ever bought 65. We are understocking this.<br/><br/>'
     '<b>3. Keep lab center engagement rings well stocked.</b> 40&#162; per dollar, $2,322 average ticket, the '
     'best finished goods category we have.<br/><br/>'
     '<b>4. Then lab accent rings, and necklaces with the rest.</b> Necklaces only pay 25&#162;, but they are '
     'the one category left that can absorb thousands at a time.<br/><br/>'
     '<b>5. Keep taking loose lab grown diamonds on memo.</b> Fastest turn in the store at 53 days and 60% '
     'margin, and they cost us nothing until they sell. That is exactly why they are not in the split.',
...
for h,t in [
 ...
 ('Loose lab grown diamonds sit outside the budget',
  'We take them on memo and owe nothing until they sell, so no part of the $25,000 needs to go to them. They stay in '
  'the ranking because the turn speed is worth seeing, but they are not in the split and their capacity column reads '
  '"on memo" rather than a dollar figure.'),
 ('Presold merchandise is excluded from the category work',
  'Nearly three in ten loose lab diamonds sell within two weeks of arriving, bought against a customer already in the '
  'store rather than stocked. Counting those would have shown 49&#162; instead of 38&#162;. Every category figure '
  'prices genuinely speculative inventory only.'),
 ('Capacity is measured, not predicted',
  '"Most we could put into it" is what a category actually cleared per six months over five and a half years, plus 50% '
  'headroom. For a line we have barely stocked, such as lab anniversary bands at 65 pieces ever, that is a floor on the '
  'real ceiling rather than a limit. Push it and watch rather than trusting the number.'),
 ('Small categories, small samples',
  ...
  'the two thinnest samples. Treat the first orders as a test, not a commitment.'),
 ('What the profit figures do and do not include',
  ...
  'assumption. Every figure is Orem store POS data.')]:
...
def page(cv,doc):
    ...
    cv.drawRightString(7.75*inch,10.55*inch,'Page %d of 4'%cv.getPageNumber())
    cv.setStrokeColor(colors.HexColor('#CCCCCC')); cv.setLineWidth(.6)
    cv.line(.75*inch,10.45*inch,7.75*inch,10.45*inch)
    cv.restoreState()

doc=BaseDocTemplate('Sierra_West_Merchandise_Report.pdf',pagesize=letter,
    leftMargin=.75*inch,rightMargin=.75*inch,topMargin=.85*inch,bottomMargin=.7*inch,
    title='Sierra West Jewelers Merchandise Return Report')
...
doc.build(story)
print('built. split profit $%s (%.0f%%), %d categories'%(format(TP,',.0f'),100*TP/B,NCAT))
